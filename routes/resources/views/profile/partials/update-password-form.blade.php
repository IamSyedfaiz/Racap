<section>
    <header>
        <h2 class="text-lg font-medium text-gray-900 pl-3">
            {{ __('Update Password') }}
        </h2>

        <p class="mt-1 text-sm text-gray-600 pl-3">
            {{ __('Ensure your account is using a long, random password to stay secure.') }}
        </p>
    </header>

    <form method="post" action="{{ route('password.update') }}" class="mt-6 space-y-6 form-group">
        @csrf
        @method('put')
